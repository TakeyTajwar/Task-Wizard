...
void tasks_navigation_help()
{
	...
	print_help(6, "<%c> to switch between windows", KB_TOGGLE);
	print_help(7, "<%c> to add a new task", KB_ADD_NEW_TASK);
	print_help(8, "<%c> to mark the task done", KB_DONE);
	print_help(9, "<%c> to skip the task", KB_SKIP);
	print_help(10, "<%c> to delete the task", KB_DELETE);
